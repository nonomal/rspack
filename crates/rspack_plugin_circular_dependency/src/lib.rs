...
pub struct CircularDependencyDetectionContext<'a> {
...
pub struct CircularDependencyPluginOptions {
  ...
  // pub on_detected:
  //   Option<Arc<dyn Fn(&CircularDependencyDetectionContext) -> Result<()> + Send + Sync>>,
  pub cwd: String,
}
...
impl CircularDependencyPlugin {
  ...
  fn is_cyclic(
    ...
    for dependency in current_module.get_dependencies() {
      let mut module_graph = compilation.get_module_graph();
      let Some(dep_module) = module_graph.get_module_by_dependency_id(dependency) else {
        continue;
      };
      ...
      if seen_modules.contains_key(&dep_module.debug_id) {
        if dep_module.debug_id == initial_module.debug_id {
          ...
          let current_resource = current_module.as_context_module().unwrap();
        ...
        self.is_cyclic(initial_module, dep_module, seen_modules, compilation)
      {
        maybe_cyclical_paths_list.insert(
          0,
          relative(&cwd, &current_module.as_normal_module().unwrap())
            .to_string_lossy()
            .into(),
        );
...
async fn optimize_modules(&self, compilation: &mut Compilation) -> Result<Option<bool>> {
  ...
  // }
  for module in compilation.get_module_graph().modules() {
    let should_skip = (module.resource_resolved_data().resource.to_string())
    ...
    if let Some(cyclical_paths_list) = maybe_cyclical_paths_list {
      ...
      // if let Some(on_detected) = &plugin.options.on_detected {
      //   match on_detected(&CircularDependencyDetectionContext {
      //     module: &module,
      ...
      if plugin.options.fail_on_error {
